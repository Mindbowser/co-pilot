...
export const providers: Partial<Record<string, ProviderInfo>> = {
  ...
  moonshot: {
    ...
    longDescription: `[Visit our documentation](https://docs.epico-pilot.dev/reference/Model%20Providers/moonshot) for information on obtaining an API key.`,
    icon: "moonshot.png",
    tags: [ModelProviderTags.RequiresApiKey],
    refPage: "moonshot",
  ...
  azure: {
    ...
    longDescription: `[Visit our documentation](https://docs.epico-pilot.dev/reference/Model%20Providers/azure) for information on obtaining an API key.
...
After it's up and running, you can start using Epico Pilot.`,
    icon: "llamacpp.png",
    tags: [ModelProviderTags.Local, ModelProviderTags.OpenSource],
    packages: openSourceModels,
    collectInputFor: [...completionParamsInputsConfigs],
...
To get started, [register](https://dataplatform.cloud.ibm.com/registration/stepone?context=wx) on watsonx SaaS, create your first project and setup an [API key](https://www.ibm.com/docs/en/mas-cd/continuous-delivery?topic=cli-creating-your-cloud-api-key).`,
  ...
  "free-trial": {
    title: "Epico Pilot limited free trial",
    provider: "free-trial",
    refPage: "freetrial",
    description:
      "New users can try out Epico Pilot for free using a proxy server that securely makes calls to OpenAI, Anthropic, or Together using our API key",
    longDescription: `New users can try out Epico Pilot for free using a proxy server that securely makes calls to OpenAI, Anthropic, or Together using our API key. If you are ready to set up a model for long-term use or have used all ${FREE_TRIAL_LIMIT_REQUESTS} free uses, you can enter your API key or use a local model.`,
    icon: "openai.png",
