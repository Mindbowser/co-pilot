...
Epico Pilot offers several way to provide your custom context to the extension.

## HTTP Context Provider

Epico Pilot can retrieve context from a custom HTTP server that you create and serve. Add the `@HTTP` context provider to your configuration like this:
...
Then, create a server that responds to requests as are made from [HttpContextProvider.ts](https://github.com/Mindbowser/co-pilot/blob/main/core/context/providers/HttpContextProvider.ts). See the `hello` endpoint in [context_provider_server.py](https://github.com/Mindbowser/co-pilot/blob/main/core/context/providers/context_provider_server.py) for an example that uses FastAPI.
...
Custom context providers can be implemented in a `config.ts` placed in your Epico Pilot global directory (`~/.epico-pilot` for MacOS, `%USERPROFILE%/.epico-pilot` for Windows). You can implement the `CustomContextProvider` interface in your `config.ts`
...
```typescript title="~/.epico-pilot/config.ts"
...
To include outside Node modules in your config.ts, run `npm install <module_name>` from the `~/.epico-pilot` directory, and then import them in config.ts.

Epico Pilot will use [esbuild](https://esbuild.github.io/) to bundle your `config.ts` and any dependencies into a single Javascript file. The exact configuration used can be found [here](https://github.com/Mindbowser/co-pilot/blob/5c9874400e223bbc9786a8823614a2e501fbdaf7/extensions/vscode/src/ideProtocol.ts#L45-L52).
...
Epico Pilot exposes an API for registering context providers from a 3rd party VSCode extension. This is useful if you have a VSCode extension that provides some additional context that you would like to use in Epico Pilot. To use this API, add the following to your `package.json`:

```json title="package.json"
{
  "extensionDependencies": ["epico-pilot.epico-pilot"]
}
```

Or install the Epico Pilot Core module from npm:

```bash
npm i @continuedev/core
```

You can add the Epico Pilot core module as a dev dependency in your `package.json`:
...
const continueExt = vscode.extensions.getExtension("Epico Pilot.epico-pilot");
...
This will register `MyCustomProvider` with Epico Pilot!
