...
In Epico Pilot, embeddings are generated during indexing and then used by [@Codebase](../deep-dives/codebase.mdx) to perform similarity search over your codebase.

You can add `embed` to a model's `roles` to specify that it can be used to embed.

:::info[Built-in model (VS Code only)]
...
<Tabs groupId="hub-config-example">
  ...
  [Voyage Code 3 Embedder Block](https://hub.epico-pilot.dev/voyageai/voyage-code-3)
  </TabItem>
...
[Transformers.js](https://huggingface.co/docs/transformers.js/index) is a JavaScript port of the popular [Transformers](https://huggingface.co/transformers/) library. It allows embeddings to be calculated entirely locally. The model used is `all-MiniLM-L6-v2`, which is shipped alongside the Epico Pilot extension.
...
<Tabs groupId="config-example">
  ...
  {/* <TabItem value="hub" label="Hub">
  [HuggingFace Text Embedder Block](https://hub.epico-pilot.dev/)
  </TabItem> */}
  <TabItem value="yaml" label="YAML">
  ```yaml title="config.yaml"
  models:
