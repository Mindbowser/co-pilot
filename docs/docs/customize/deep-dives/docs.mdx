---
description: Learn how to access and search your project's documentation directly within Epico Pilot
keywords:
  [
    documentation,
    indexing,
...
The [`@Docs` context provider](customize/context-providers.mdx#docs) allows you to efficiently reference documentation directly within Epico Pilot.
...
In VS Code, Epico Pilot offers a selection of pre-indexed documentation sites for popular frameworks and libraries. You can view the list of [available pre-indexed sites and request additions here](https://github.com/Mindbowser/co-pilot/blob/main/core/indexing/docs/preIndexedDocs.ts).
...
@Docs uses [`docs` blocks](../../hub/blocks/block-types.md#docs) in Assistants from the hub. Visit the hub to [explore `docs` blocks](https://hub.epico-pilot.dev/explore/docs) or [create your own](https://hub.epico-pilot.dev/new?type=block&blockType=docs).
...
By default, Epico Pilot crawls documentation sites using a specialized crawling service that provides the best experience for most users and documentation sites.
...
- Epico Pilot does not automatically re-index your docs. Use `Click to re-index` to trigger a reindex for a specific source
...
You can also use the following IDE command to force a re-index of all docs: `Epico Pilot: Docs Force Re-Index`.
