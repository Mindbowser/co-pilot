...
<Tabs groupId="hub-config-example">
  ...
  [Mistral Codestral model block](https://hub.epico-pilot.dev/mistral/codestral)
  </TabItem>
...
<Tabs groupId="hub-config-example">
  <TabItem value="hub" label="Hub">
  [Ollama Qwen 2.5 Coder 1.5B model block](https://hub.epico-pilot.dev/ollama/qwen2.5-coder-1.5b)
  </TabItem>
  <TabItem value="yaml" label="YAML">
  ```yaml title="config.yaml"
  models:
...
Explore autocomplete model configurations on [the hub](https://hub.epico-pilot.dev/explore/models?roles=autocomplete)
...
5. Check the output of the logs to find any potential errors: <kbd>cmd/ctrl</kbd> + <kbd>shift</kbd> + <kbd>P</kbd> -> "Toggle Developer Tools" -> "Console" tab in VS Code, ~/.epico-pilot/logs/core.log in JetBrains.
...
Click the "Epico Pilot" button in the status panel at the bottom right of the screen. The checkmark will become a "cancel" symbol and you will no longer see completions. You can click again to turn it back on.

Alternatively, open VS Code settings, search for "Epico Pilot" and uncheck the box for "Enable Tab Autocomplete".

You can also use the default shortcut to disable autocomplete directly using a chord: press and hold <kbd>ctrl/cmd</kbd> + <kbd>K</kbd> (epico-pilot holding <kbd>ctrl/cmd</kbd>) and press <kbd>ctrl/cmd</kbd> + <kbd>A</kbd>. This will turn off autocomplete without navigating through settings.

#### JetBrains

Open Settings -> Tools -> Epico Pilot and uncheck the box for "Enable Tab Autocomplete".
