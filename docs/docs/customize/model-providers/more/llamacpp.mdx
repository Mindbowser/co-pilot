import TabItem from "@theme/TabItem";
import Tabs from "@theme/Tabs";

# LlamaCpp

Run the llama.cpp server binary to start the API server. If running on a remote server, be sure to set host to 0.0.0.0:

```bash
.\server.exe -c 4096 --host 0.0.0.0 -t 16 --mlock -m models\meta\llama\codellama-7b-instruct.Q8_0.gguf
```

After it's up and running, change `~/.epico-pilot/config.json` to look like this:

<Tabs groupId="config-example">
  <TabItem value="yaml" label="YAML">
  ```yaml title="config.yaml"
  models:
    - name: Llama CPP
      provider: llama.cpp
      model: MODEL_NAME
      apiBase: http://localhost:8080
  ```
  </TabItem>
  <TabItem value="json" label="JSON">
  ```json title="config.json"
  {
    "models": [
      {
        "title": "Llama CPP",
        "provider": "llama.cpp",
        "model": "MODEL_NAME",
        "apiBase": "http://localhost:8080"
      }
    ]
  }
  ```
  </TabItem>
</Tabs>

[View the source](https://github.com/Mindbowser/co-pilot/blob/main/core/llm/llms/LlamaCpp.ts)
