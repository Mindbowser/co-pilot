...
[Msty](https://msty.app/) is an application for Windows, Mac, and Linux that makes it really easy to run online as well as local open-source models, including Llama-2, DeepSeek Coder, etc. No need to fidget with your terminal, run a command, or anything. Just download the app from the website, click a button, and you are up and running. Epico Pilot can then be configured to use the `Msty` LLM class:
...
[View the source](https://github.com/Mindbowser/co-pilot/blob/main/core/llm/llms/Msty.ts)
