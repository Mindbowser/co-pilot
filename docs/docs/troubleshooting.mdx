...
The Epico Pilot VS Code extension is currently in beta, and the JetBrains extension is in Alpha. If you are having trouble, please follow the steps below.
...
1. <kbd>cmd</kbd> + <kbd>shift</kbd> + <kbd>P</kbd> for MacOS or <kbd>ctrl</kbd> + <kbd>
     shift
   </kbd> + <kbd>P</kbd> for Windows
...
2. In the dropdown, select "Epico Pilot - LLM Prompts/Completions"
3. View the exact prompts that were sent to the LLM and the completions received

### JetBrains

Open `~/.epico-pilot/logs/core.log` to view the logs for the Epico Pilot plugin. The most recent logs are found at the bottom of the file.
...
We are constantly making fixes and improvements to Epico Pilot, but the latest changes remain in a "pre-release" version for roughly a week so that we can test their stability. If you are experiencing issues, you can try the pre-release by going to the Epico Pilot extension page in VS Code and selecting "Switch to Pre-Release" as shown below.
...
For VS Code, All versions are hosted on the Open VSX Registry [here](https://open-vsx.org/extension/Epico Pilot/epico-pilot). Once you've downloaded the extension, which will be a .vsix file, you can install it manually by following the instructions [here](https://code.visualstudio.com/docs/editor/extension-gallery#_install-from-a-vsix).

You can find older versions of the JetBrains extension on their [marketplace](https://plugins.jetbrains.com/plugin/22707-epico-pilot), which will walk you through installing from disk.

## Keyboard shortcuts not resolving

If your keyboard shortcuts are not resolving, you may have other commands that are taking precedence over the Epico Pilot shortcuts. You can see if this is the case, and change your shortcut mappings, in the configuration of your IDE.
...
Epico Pilot can be used in [code-server](https://coder.com/), but if you are running across an error in the logs that includes "This is likely because the editor is not running in a secure context", please see [their documentation on securely exposing code-server](https://coder.com/docs/code-server/latest/guide#expose-code-server).

### I installed Epico Pilot, but don't see the sidebar window

By default the Epico Pilot window is on the left side of VS Code, but it can be dragged to right side as well, which we recommend in our tutorial. In the situation where you have previously installed Epico Pilot and moved it to the right side, it may still be there. You can reveal Epico Pilot either by using cmd/ctrl+L or by clicking the button in the top right of VS Code to open the right sidebar.
...
If you have entered a valid API key and model, but are still getting a 404 error from OpenRouter, this may be because models that do not support function calling will return an error to Epico Pilot when a request is sent.
Example error:
...
This can be accomplished using the following command: `Epico Pilot: Rebuild codebase index`.

### Android Studio - "Nothing to show" in Chat

This can be fixed by selecting `Actions > Choose Boot runtime for the IDE` then selecting the latest version, and then restarting Android Studio. [See this thread](https://github.com/Mindbowser/co-pilot/issues/596#issuecomment-1789327178) for details.
