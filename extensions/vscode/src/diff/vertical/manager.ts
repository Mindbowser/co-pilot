...
export class VerticalDiffManager {
  ...
  // Listener for user doc changes is disabled during updates to the text document by epico-pilot
  ...
  clearForfileUri(fileUri: string | undefined, accept: boolean) {
    ...
    vscode.commands.executeCommand("setContext", "epico-pilot.diffVisible", false);
  }
  ...
  async acceptRejectVerticalDiffBlock(
    ...
    // Disable listening to file changes while epico-pilot makes changes
    this.disableDocumentChangeListener();
  ...
  async streamDiffLines(
    ...
    vscode.commands.executeCommand("setContext", "epico-pilot.diffVisible", true);
  ...
  async streamEdit(
    ...
    if (existingHandler) {
      ...
      // but extra delta was added for each line generated by Epico Pilot
      // Before adding this back, we need to distinguish between human and Epico Pilot
