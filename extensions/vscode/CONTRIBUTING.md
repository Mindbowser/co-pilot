...
This is the Epico Pilot VS Code Extension. Its primary jobs are

1. Implement the IDE side of the Epico Pilot IDE protocol, allowing a Epico Pilot server to interact natively in an IDE. This happens in `src/continueIdeClient.ts`.
2. Open the Epico Pilot React app in a side panel. The React app's source code lives in the `gui` directory. The panel is opened by the `continue.openContinueGUI` command, as defined in `src/commands.ts`.
